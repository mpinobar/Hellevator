...
public class Fire : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(m_detections.Count > 0)
        {
            ...
            {
                if (m_detections[i] == null)
                    m_detections.RemoveAt(i);
            }
            if (m_impacts.Length > 0)
            {
                for (int i = 0; i < m_impacts.Length; i++)
                {
                    //Debug.LogError(m_impacts[i].transform.name);
                    ...
                    }else if (m_impacts[i].transform != null && m_impacts[i].transform.TryGetComponent(out DestructiblePlatform platform))
                    {
                        m_impacts[i].transform.position -= Vector3.up * Time.deltaTime;
                    }
                }
            }
            else
        ...
        else
        {
            for (int i = 0; i < m_impacts.Length; i++)
            {
                if (m_impacts[i].transform != null && m_impacts[i].transform.TryGetComponent(out DestructiblePlatform platform))
                {
                    m_impacts[i].transform.position -= Vector3.up * Time.deltaTime;
                }
            }
